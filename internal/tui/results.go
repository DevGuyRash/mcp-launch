...
func (m resultsModel) Init() tea.Cmd {
    var cmds []tea.Cmd
    if m.ch != nil { cmds = append(cmds, waitLog(m.ch)) }
    if m.upch != nil { cmds = append(cmds, waitItem(m.upch)) }
...
func (m resultsModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.KeyMsg:
        ...
        case "l":
            ...
            return m, nil
        case "j":
            if m.showLogs && m.logOffset < len(m.logs) { m.logOffset++ }
            return m, nil
        case "k":
            if m.showLogs && m.logOffset > 0 { m.logOffset-- }
            return m, nil
        case "J":
            if m.showLogs { m.logOffset += 5; if m.logOffset > len(m.logs) { m.logOffset = len(m.logs) } }
            return m, nil
        case "K":
            if m.showLogs { m.logOffset -= 5; if m.logOffset < 0 { m.logOffset = 0 } }
            return m, nil
        case "w":
        ...
        case "/":
            ...
            return m, nil
        case "n":
            if len(m.searchIdxs) > 0 { m.jumpToResult(m.searchPos+1) }
            return m, nil
        case "N":
            if len(m.searchIdxs) > 0 { m.jumpToResult(m.searchPos-1) }
            return m, nil
    ...
    case itemUpdateMsg:
        if v.Idx >= 0 && v.Idx < len(m.items) {
            ...
            if v.OpenAPI != "" { it.OpenAPI = v.OpenAPI }
            if v.APIKeyRaw != "" { it.APIKeyRaw = v.APIKeyRaw }
            if v.APIKeyMasked != "" { it.APIKeyMasked = v.APIKeyMasked }
            if v.ConfigPath != "" { it.ConfigPath = v.ConfigPath }
            if v.ServersCount >= 0 { it.ServersCount = v.ServersCount }
            if v.Endpoints >= 0 { it.Endpoints = v.Endpoints }
            if v.PerServerCounts != nil { it.PerServerCounts = v.PerServerCounts }
            if v.LongDescCounts != nil { it.LongDescCounts = v.LongDescCounts }
            m.items[v.Idx] = it
        }
        return m, waitItem(m.upch)
    case tea.WindowSizeMsg:
        if v.Width > 0 { m.width = v.Width }
        return m, nil
    }
    return m, nil
}
...
func (m resultsModel) View() string {
    ...
    for i, it := range m.items {
        ...
        warn := ""
        if it.Endpoints > 30 { warn = "  ⚠ OVER 30-limit" } else if it.Endpoints >= 28 { warn = "  ⚠ near 30" }
        ...
        if len(it.PerServerCounts) > 0 {
            ...
            for name, cnt := range it.PerServerCounts {
                long := 0
                if it.LongDescCounts != nil { long = it.LongDescCounts[name] }
                suffix := ""
                if long > 0 { suffix = "  ⚠ tool descriptions >300" }
    ...
    status := ""
    if m.searching { status = fmt.Sprintf("  /%s", m.searchBuf) } else if len(m.searchIdxs) > 0 { status = fmt.Sprintf("  [%d/%d]", m.searchPos+1, len(m.searchIdxs)) }
    b.WriteString("(l) logs  (j/k/J/K) scroll  (w) wrap  (/) search  (n/N) next/prev  (S) save" + status + "  (q) quit\n\n")
    if m.showLogs {
        ...
        for i, ln := range m.logs[start:end] {
            ...
            if m.wrapLogs {
                ...
                r := []rune(ln)
                if len(r) > avail { ln = string(r[:avail-1]) + "…" }
                lines = append(lines, m.highlight(ln, idx))
            }
        }
...
func (m *resultsModel) jumpToResult(pos int) {
    if len(m.searchIdxs) == 0 { return }
    if pos < 0 { pos = len(m.searchIdxs) - 1 }
    if pos >= len(m.searchIdxs) { pos = 0 }
    m.searchPos = pos
    ...
    if m.logOffset < 0 { m.logOffset = 0 }
}
...
func (m *resultsModel) highlight(s string, idx int) string {
    q := strings.ToLower(strings.TrimSpace(m.searchBuf))
    if q == "" || len(m.searchIdxs) == 0 { return s }
    if !containsIndex(m.searchIdxs, idx) { return s }
    ...
    for {
        ...
        if off >= len(out) { break }
    }
    return out
}

func containsIndex(a []int, x int) bool { for _, v := range a { if v == x { return true } } ; return false }

func (m *resultsModel) saveLogs() error {
